...
use App\Exports\Commons\DegreeExport;
...
class DegreeController extends Controller
{
    ...
    private $_degreeServiceInterface;
     ...
     * @param DegreeServiceInterface $degreeServiceInterface
     */
    public function __construct(DegreeServiceInterface $degreeServiceInterface)
    {
        $this->_degreeServiceInterface = $degreeServiceInterface;
    }
    ...
    public function getDegreeList(Request $request)
    {
        ...
        return $this->getListJson($isActive,
            [$this->_degreeServiceInterface, 'degreeList'],
     ...
     *                      example="keyword"
     *                  ),
     *                  @OA\Property(property="is_active", ref="#/components/schemas/DegreeEloquent/properties/is_active")
     *              )
     *          )
     *     ),
     *     @OA\Response(
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function postDegreeListSearch(Request $request)
    {
        $isActive = $request->input('is_active');

        return $this->getListSearchJson($request, $isActive,
            [$this->_degreeServiceInterface, 'degreeListSearch'],
            function (Collection $entities) {
                $rowJsonData = new Collection();
     ...
     * @return mixed
     */
    public function postDegreePageSearch(Request $request)
    {
        $isActive = $request->input('is_active');

        return $this->getPagedSearchJson($request, $isActive,
            [$this->_degreeServiceInterface, 'degreePageSearch'],
            function (Collection $entities) {
                $rowJsonData = new Collection();
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function getDegreeDetail(int $id)
    {
        return $this->getDetailObjectJson($id,
            [$this->_degreeServiceInterface, 'find'],
            function ($entity) {
                $rowJsonData = new Collection();
     ...
     * @return mixed
     */
    public function postDegreeCreate(Request $request)
    {
        $degree = $this->_degreeServiceInterface->newInstance();

        $degree->name = $request->input('name');
        $degree->slug = $request->input('slug');
        $degree->description = $request->input('description');

        $this->setRequestAuthor($degree);

        $response = $this->_degreeServiceInterface->create($degree);
        $degreeCreated = $response->getObject();

        if (!$response->isSuccess()) {
            return $this->getBasicErrorJson($response);
        }

        return $this->getBasicSuccessJson($response, $degreeCreated);
    }
     ...
     *          @OA\MediaType(
     *              mediaType="application/json",
     *              @OA\Schema(ref="#/components/schemas/UpdateDegreeEloquent")
     *          )
     *     ),
     *     @OA\Response(
     *          response=200,
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function putDegreeUpdate(Request $request)
    {
        $degree = $this->_degreeServiceInterface->find($request->input('id'));

        $result = $degree->getObject();
        ...
        $response = $this->_degreeServiceInterface->update($result);
        $degreeUpdated = $response->getObject();

        if (!$response->isSuccess()) {
            return $this->getBasicErrorJson($response);
        }

        return $this->getBasicSuccessJson($response, $degreeUpdated);
    }

    /**
     * @OA\Delete(
     *     path="/degree/delete/{id}",
    ...
    {
        $degree = $this->_degreeServiceInterface->find($id);

        $result = $degree->getObject();

        $response = $this->_degreeServiceInterface->delete($result);
     ...
     *
     * @param Request $request
     * @return GenericObjectResponse|\Illuminate\Http\JsonResponse|mixed
     */
    public function deleteBulkDegree(Request $request)
    {
        $ids = $request->input('ids');

        $response = $this->_degreeServiceInterface->deleteBulk($ids);
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function putDegreeActive(Request $request)
    {
        $degree = $this->_degreeServiceInterface->find($request->input('id'));
        ...
        $response = $this->_degreeServiceInterface->degreeSetActive($result);
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function getDegreeSlug(string $name)
    {
        return $this->getSlugObjectJson($name,
            [$this->_degreeServiceInterface, 'degreeSlug'],
            function ($entity) {
                $rowJsonData = new Collection();
    ...
    /**
     * @OA\Post(
     *     path="/degree/list-search/export",
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function postDegreeListSearchExport(Request $request)
    {
    ...
    private function generateExcel(Collection $entities, string $ext = null) {
        ...
        if (Excel::store(new DegreeExport($entities), $path . $file)) {
    ...
    private function generatePDF(Collection $entities, string $ext = null) {
        ...
        if (PDF::loadView('exports.commons.degree', ['degrees' => $entities])
                                                   ...
                                                   callable $dtoObjectToJsonMethod)
    {
        $parameter = $this->generateListSearchParameter($request);
        $response = $searchMethod($parameter, $isActive);
        $itemJsonData = $dtoObjectToJsonMethod($response->getDtoCollection(), $export);
