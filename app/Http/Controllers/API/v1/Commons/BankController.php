...
use App\Exports\Commons\BankExport;
...
class BankController extends Controller
{
     ...
     * @param BankServiceInterface $bankServiceInterface
     */
    public function __construct(BankServiceInterface $bankServiceInterface)
    {
        $this->_bankServiceInterface = $bankServiceInterface;
    }
    ...
    public function getBankList(Request $request)
    {
        ...
        return $this->getListJson($isActive,
            [$this->_bankServiceInterface, 'bankList'],
     ...
     *                      example="keyword"
     *                  ),
     *                  @OA\Property(property="is_active", ref="#/components/schemas/BankEloquent/properties/is_active")
     *              )
     *          )
     *     ),
     *     @OA\Response(
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function postBankListSearch(Request $request)
    {
        $isActive = $request->input('is_active');

        return $this->getListSearchJson($request, $isActive,
            [$this->_bankServiceInterface, 'bankListSearch'],
            function (Collection $entities) {
                $rowJsonData = new Collection();
     ...
     * @return mixed
     */
    public function postBankPageSearch(Request $request)
    {
        $isActive = $request->input('is_active');

        return $this->getPagedSearchJson($request, $isActive,
            [$this->_bankServiceInterface, 'bankPageSearch'],
            function (Collection $entities) {
                $rowJsonData = new Collection();
    ...
    public function getBankDetail(int $id)
    {
        return $this->getDetailObjectJson($id,
            [$this->_bankServiceInterface, 'find'],
            function ($entity) {
                $rowJsonData = new Collection();
    ...
    {
        $bank = $this->_bankServiceInterface->newInstance();

        $bank->name = $request->input('name');
        $bank->slug = $request->input('slug');
        $bank->description = $request->input('description');

        $this->setRequestAuthor($bank);

        $response = $this->_bankServiceInterface->create($bank);
        $bankCreated = $response->getObject();

        if (!$response->isSuccess()) {
            return $this->getBasicErrorJson($response);
        }

        return $this->getBasicSuccessJson($response, $bankCreated);
    }
     ...
     *          @OA\MediaType(
     *              mediaType="application/json",
     *              @OA\Schema(ref="#/components/schemas/UpdateBankEloquent")
     *          )
     *     ),
     *     @OA\Response(
     *          response=200,
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function putBankUpdate(Request $request)
    {
        $bank = $this->_bankServiceInterface->find($request->input('id'));

        $result = $bank->getObject();
        ...
        $response = $this->_bankServiceInterface->update($result);
        $bankUpdated = $response->getObject();

        if (!$response->isSuccess()) {
            return $this->getBasicErrorJson($response);
        }

        return $this->getBasicSuccessJson($response, $bankUpdated);
    }
    ...
    {
        $bank = $this->_bankServiceInterface->find($id);

        $result = $bank->getObject();

        $response = $this->_bankServiceInterface->delete($result);
     ...
     */
    public function deleteBulkBank(Request $request)
    {
        $ids = $request->input('ids');

        $response = $this->_bankServiceInterface->deleteBulk($ids);
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function putBankActive(Request $request)
    {
        $bank = $this->_bankServiceInterface->find($request->input('id'));
        ...
        $response = $this->_bankServiceInterface->bankSetActive($result);
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function getBankSlug(string $name)
    {
        return $this->getSlugObjectJson($name,
            [$this->_bankServiceInterface, 'bankSlug'],
            function ($entity) {
                $rowJsonData = new Collection();
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function postBankListSearchExport(Request $request)
    {
    ...
    private function generateExcel(Collection $entities, string $ext = null) {
        ...
        if (Excel::store(new BankExport($entities), $path . $file)) {
    ...
    private function generatePDF(Collection $entities, string $ext = null) {
        ...
        if (PDF::loadView('exports.commons.bank', ['banks' => $entities])
